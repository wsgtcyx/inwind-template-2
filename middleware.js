...
export function middleware(request) {
	...
	// 检查是否已经包含语言代码
	const isExit = Object.keys(localeNames).some((locale) => pathname.startsWith(`/${locale}/`) || pathname === `/${locale}`);
...
export const config = {
	matcher: [ "/((?!api|_next/static|_next/image|terms|.*\\.(?:txt|xml|ico|png|jpg|jpeg|svg|gif|webp|js|css|woff|woff2|ttf|eot)).*)"],
};
